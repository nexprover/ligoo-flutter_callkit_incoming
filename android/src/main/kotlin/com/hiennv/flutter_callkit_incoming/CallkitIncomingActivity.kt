    ...
    companion object {
        ...
        fun getIntentEnded(context: Context, isAccepted: Boolean, declineClicked: Boolean, data: Bundle): Intent {
            ...
            if (declineClicked) {
                ...
                ApiService().sendDeclineActionToApi(callId, isDev)
            }
            return intent
        }
    }
        ...
        try {
            ...
            val thread = Thread {
                try {
                    ...
                    if (isDev) {
                        url = "https://dev-api.ligoo.live/v1/public/call/${callId}"
                    }
                    val (request, response, result) = Fuel.put(url).response()
